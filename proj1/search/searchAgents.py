...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.boolean = [False, False, False, False]
    def getStartState(self):
        """
...
def cornersHeuristic(state, problem):
    ...
    if False not in state[1]:
        ...
        for i in range(count):
            near_corners.append(util.manhattanDistance(corners_left[i], state[0]))
            



        if len(near_corners) >= 1:
            to_nearest = min(near_corners)

        dist = util.manhattanDistance(corners[0], corners[1])

        return to_nearest + dist * (count - 1)
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        dist_to_foods.append(mazeDistance(food, position, problem.startingGameState))
        food_count += 1

    dist_max = max(dist_to_foods)
    dist_min = min(dist_to_foods)
    return max(dist_max, food_count * dist_min )
