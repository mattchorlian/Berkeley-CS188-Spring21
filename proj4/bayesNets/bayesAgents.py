...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    p = 0
    #I think this may be the best way to get the best assignment
    #np.argmax is off the table but would be useful here
    for assignment in factor.getAllPossibleAssignmentDicts():
        #update best assignment
        if factor.getProbability(assignment) > p:
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        prob_food_left = [factor.getProbability(assignment) for assignment in assignments if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL]
        prob_food_right = [factor.getProbability(assignment) for assignment in assignments if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL]

        leftExpectedValue = WON_GAME_REWARD * prob_food_left[0] + GHOST_COLLISION_REWARD * prob_food_right[0]
        rightExpectedValue = WON_GAME_REWARD * prob_food_right[0] + GHOST_COLLISION_REWARD * prob_food_left[0]
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # iterate through new evidence and update expected value according to linearity of expectation
        for p, e in self.getExplorationProbsAndOutcomes(evidence):
            max_val = max(self.computeEnterValues(e, enterEliminationOrder))
            expectedValue += p * max_val
        "*** END YOUR CODE HERE ***"
