...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        factors_to_join = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #just following the pseudocode in note 4 page 6
        #in the code SUM-OUT is analagous to our eliminate function
        for var in eliminationOrder:
            factors_to_join, joined_factor = joinFactorsByVariable(factors_to_join, var)
            #if this factor has no conditioned variables, assume it is 1 and go to the next factor
            if len(joined_factor.unconditionedVariables()) <= 1:
                continue
            eliminated_factor = eliminate(joined_factor, var)
            factors_to_join.append(eliminated_factor)
        return normalize(joinFactors(factors_to_join))
